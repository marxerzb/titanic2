...
```{r}
#???
 modellek <- caretList(as.factor(Survived)~., data=traink, methodList=c("rpart","rf","gbm"), trControl=trainControl(method="cv", number=5, savePredictions="final", classProbs=TRUE))
 szuper <- caretStack(modellek, method="logreg", trControl=trainControl(method="cv", number=5, savePredictions="final", classProbs=TRUE))
 szuper
```
